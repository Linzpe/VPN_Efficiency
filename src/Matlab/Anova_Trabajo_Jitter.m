...
for i = 1:4
    ...
    for j = 1:length(uy)
        ...
        for k = 1:length(aux)
            ...
            % así sucesivamente.
            res2_concatenado = [res2_concatenado jitter(aux(k),:)];
        end
...
positions = find(jitter_vector>60);
jitter_vector(positions) = [];
...
[P,T,STATS_B3]=anovan(rank_transform(jitter_vector),factores_vector,'varnames',labels, 'model', 'interaction');
